...
class DeliveryReconciliationService{
    public function __construct(private GmailAdapter $gmail,private OutboxService $outbox){}
    public function reconcile(GalikaApplication $a):array{
        ...
        if($hard){$route->update(['state'=>'HARD_BOUNCED','last_reconciled_at'=>now(),'hard_bounces'=>$route->hard_bounces+1,'do_not_retry'=>true]);$this->event($a,'HARD_BOUNCE',$email,$hard['id'],$hard['code'],$hard['body'],'INVALIDATES_ROUTE');$a->update(['delivery_state'=>'HARD_BOUNCED','delivery_evidence'=>'Permanent DSN '.$hard['id'],'delivery_reconciled_at'=>now(),'status'=>'FAILED_RETRYING','failure_class'=>'OTHER']);$this->outbox->publish('delivery.hard_bounce',['email'=>$email,'dsn_id'=>$hard['id'],'code'=>$hard['code']],$a->user_id,'application',$a->id);return ['state'=>'HARD_BOUNCED'];}
        if($soft){$route->update(['state'=>'SENT_PENDING','last_reconciled_at'=>now()]);$this->event($a,'SOFT_BOUNCE',$email,$soft['id'],$soft['code'],$soft['body'],'NON_TERMINAL');$a->update(['delivery_state'=>'SENT_PENDING','delivery_evidence'=>'Temporary DSN '.$soft['id'],'delivery_reconciled_at'=>now()]);$this->outbox->publish('delivery.soft_bounce',['email'=>$email,'dsn_id'=>$soft['id'],'code'=>$soft['code']],$a->user_id,'application',$a->id);return ['state'=>'SENT_PENDING'];}
