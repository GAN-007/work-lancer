...
class GalikaHealthController extends Controller {
 public function __invoke(Request $request):JsonResponse {
  ...
  return response()->json(['ok'=>(bool)$healthy,'worker'=>$h,'pending_outbox'=>DB::table('galika_outbox')->whereIn('status',['PENDING','RETRY'])->count(),'queued_work'=>DB::table('galika_execution_work_items')->whereIn('status',['QUEUED','RETRY'])->count()],$healthy?200:503);
 }
}